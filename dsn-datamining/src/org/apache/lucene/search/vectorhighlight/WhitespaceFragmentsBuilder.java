...
public class WhitespaceFragmentsBuilder extends BaseFragmentsBuilder {
	...
	protected String makeFragment(StringBuilder buffer, int[] index,
			String[] values, WeightedFragInfo fragInfo) {
		...
		for (SubInfo subInfo : fragInfo.subInfos) {
			for (Toffs to : subInfo.termsOffsets) {
				fragment.append(src.substring(srcIndex, to.startOffset - s))
						...
						.append(src.substring(to.startOffset - s, to.endOffset
	...
	protected String getFragmentSource(StringBuilder buffer, int[] index,
			String[] values, WeightedFragInfo fragInfo) {
		...
		while (fragInfo.startOffset > 0 && words > 0) {
			if (!Character.isWhitespace(buffer.charAt(fragInfo.startOffset)))
		...
		while (fragInfo.endOffset < buffer.length()
				&& !Character.isWhitespace(buffer
						.charAt(fragInfo.endOffset - 1))) {
			fragInfo.endOffset++;
		}
		return buffer.substring(fragInfo.startOffset,
				buffer.length() < fragInfo.endOffset ? buffer.length()
						: fragInfo.endOffset);
	}
